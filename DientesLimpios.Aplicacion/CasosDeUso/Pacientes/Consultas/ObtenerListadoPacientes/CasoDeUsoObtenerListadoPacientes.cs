...
namespace DientesLimpios.Aplicacion.CasosDeUso.Pacientes.Consultas.ObtenerListadoPacientes
{
                                                                       ...
                                                                       PaginadoDTO<PacienteListadoDTO>>
    {
        ...
        {
            var pacientes = await repositorio.ObtenerFiltrado(request); // Obtiene todos los pacientes desde el repositorio
            var totalPacientes = await repositorio.ObtenerCantidadTotalRegistros(); // Obtiene la cantidad total de pacientes
            var pacientesDTO = pacientes.Select(paciente => paciente.ADto()).ToList(); // Mapea las entidades a DTOs
